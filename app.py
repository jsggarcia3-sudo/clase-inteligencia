        ...
        if st.session_state.get('es_admin', False):
            ...
            try:
                ...
                if not df_raw.empty:
                    ...
                    # 1. Convertir fecha a objeto datetime para ordenar
                    df_raw['fecha'] = pd.to_datetime(df_raw['fecha'])
                    
                    # 2. Quedarnos solo con el último intento de cada agente por módulo
                    # Esto evita que un agente que hizo el examen 3 veces cuente 3 veces en el promedio
                    df_all = df_raw.sort_values('fecha').drop_duplicates(subset=['funcionario', 'modulo'], keep='last')
                    
                    # Formatear fecha para mostrar
                    df_all['fecha_str'] = df_all['fecha'].dt.strftime('%Y-%m-%d %H:%M')
                    
                    # --- MÉTRICAS KPI ---
                    promedio = df_all['nota'].mean()
                    total_intentos = len(df_raw) # El total real de exámenes hechos
                    agentes_unicos = df_all['funcionario'].nunique()
                    aprobados = len(df_all[df_all['nota'] >= 70])
                    porcentaje_exito = (aprobados / len(df_all)) * 100

                    # Estilos de tarjetas (Se mantienen tus estilos Pro)
                    st.markdown("""
                        <style>
                        .metric-card {
                        ...
                        .metric-value { color: #D4AF37; font-size: 3.5rem; font-weight: 900; margin: 0; }
                        .metric-title { color: #8b949e; font-size: 1rem; font-weight: bold; text-transform: uppercase; }
                        </style>
                    """, unsafe_allow_html=True)

                    m1, m2, m3, m4 = st.columns(4)
                    with m1:
                        st.markdown(f'<div class="metric-card"><p class="metric-title">Agentes</p><p class="metric-value">{agentes_unicos}</p></div>', unsafe_allow_html=True)
                    with m2:
                    ...
                    with m4:
                        st.markdown(f'<div class="metric-card"><p class="metric-title">Exámenes</p><p class="metric-value">{total_intentos}</p></div>', unsafe_allow_html=True)

                    # --- PESTAÑAS ---
                    tab_rend, tab_detalles = st.tabs(["📊 Análisis Limpio", "🔍 Historial Completo"])

                    with tab_rend:
                        c1, c2 = st.columns([1.2, 0.8])
                        with c1:
                            st.subheader("Promedio por Componente (Últimos Intentos)")
                        ...
                        with c2:
                            ...
                            df_all['Estado'] = df_all['nota'].apply(lambda x: 'Aprobado' if x >= 70 else 'Repaso')
                            st.bar_chart(df_all['Estado'].value_counts(), color="#2ecc71")

                    with tab_detalles:
                        st.subheader("🔍 Buscador de Registros")
                        search = st.text_input("Filtrar por nombre...")
                        
                        # Aquí mostramos el RAW (todo el historial) por si el admin quiere ver intentos fallidos previos
