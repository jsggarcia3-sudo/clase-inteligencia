...
def login():
    ...
    with col2:
        ...
        nombre_ingresado = st.text_input("Nombre Completo (Ej: Juan Pérez)")
...
if not st.session_state['autenticado']:
    ...
    with st.sidebar:
        ...
        st.divider()
        seccion = st.radio("Ir a:", ["🏠 Inicio", "📚 Módulos", "📊 Progreso General"])
    ...
    if seccion == "🏠 Inicio":
        st.title(f"Panel de Control - {st.session_state['agente_nombre']}")
        st.write("Bienvenido al sistema de gestión académica de la DIPOL.")
        ...
        if seleccion == "Módulo 1: Conceptualización de Inteligencia":
            if not st.session_state['modo_examen']:
                ...
                with st.form(key="form_eval"):
                    p1 = st.radio("1. Función principal de la inteligencia:", ["Duda", "Asesoramiento para reducir incertidumbres", "Fuerza"])
                    p2 = st.radio("2. Nivel para planes nacionales:", ["Estratégica", "Operacional", "Táctica"])
                    p3 = st.radio("3. Nivel enfocado en capacidades del objetivo e inmediatas:", ["Operacional", "Estratégica", "Táctica"])
                    ...
                    if st.form_submit_button("GUARDAR NOTA"):
                        aciertos = sum([p1=="Asesoramiento para reducir incertidumbres", p2=="Estratégica", p3=="Táctica", p4=="Habilidades concretas y experiencia sensorial", p5=="Conocimiento para seguridad y convivencia ciudadana"])
                        nota = (aciertos / 5) * 100
                        try:
    ...
    elif seccion == "📊 Progreso General":
        st.title("📊 Dashboard de Rendimiento Académico")
        try:
            query = text("SELECT funcionario, modulo, nota, fecha FROM calificaciones ORDER BY fecha DESC")
            df = pd.read_sql(query, engine)
            
            if not df.empty:
                # METRICAS DEL DASHBOARD
                c1, c2, c3 = st.columns(3)
                c1.metric("Total de Evaluaciones", len(df))
                c2.metric("Promedio General", f"{df['nota'].mean():.1f}%")
                c3.metric("Estudiantes Activos", df['funcionario'].nunique())

                st.divider()

                # GRÁFICO DE DESARROLLO
                st.subheader("📈 Desarrollo por Estudiante (Últimas Notas)")
                # Agrupamos por funcionario para ver su promedio o última nota
                df_chart = df.groupby('funcionario')['nota'].mean().reset_index()
                st.bar_chart(df_chart.set_index('funcionario'))

                # TABLA DETALLADA
                st.subheader("📋 Registro Completo de Estudiantes")
